package org.pixelheroes.entity;

import com.mojang.nbt.NBTCompoundTag;
import org.pixelheroes.blocks.Block;
import org.pixelheroes.maths.AABB;
import org.pixelheroes.maths.Quaternion;
import org.pixelheroes.maths.Vector3;
import org.pixelheroes.utils.CollisionInfos;
...
public abstract class Entity implements ILocatable {
		...
		for (int x = startX; x <= endX; x++) {
			for (int y = startY; y <= endY; y++) {
				for (int z = startZ; z <= endZ; z++) {
					if (worldObj.getChunk(x, y, z) == null) return false;
	...
	public CollisionInfos getObjectInFront(float maxDist) {
		collInfos.type = CollisionInfos.CollisionType.NONE;
	...
	public void readFromNBT(NBTCompoundTag compound) {
		if (compound.contains("uuid")) uuid = UUID.fromString(compound.getString("uuid"));
	...
	public void writeToNBT(NBTCompoundTag compound) {
		if (uuid != null) compound.putString("uuid", uuid.toString());
	...
	public void onInteract(org.pixelheroes.inventory.Stack itemStack) {
		;
	}
}
