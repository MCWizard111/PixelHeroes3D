...
import org.pixelheroes.IParticleHandler;
...
public class World implements IParticleHandler, IAudioHandler {
	...
	public void update(double delta) {
		while (!spawingQueue.isEmpty()) entities.add(spawingQueue.remove(0));
	...
	public void setBlockState(int x, int y, int z, BlockState state, IBlockStateValue value, boolean notify) {
		...
		if (c != null) {
			...
			if (notify) updateBlockNeighbors(x, y, z, false);
		}
	}
	...
	public void performRayCast(Entity sender, CollisionInfos infos, float maxDist) {
				...
				if (blockBB != null) {
					if (blockBB.intersectAABB(translatedRay)) {
						...
						if (absx > absy && absx > absz) {
							if (diff.getX() > 0) infos.side = EnumSide.WEST;
							else infos.side = EnumSide.EAST;
						}
						if (absy > absx && absy > absz) {
							if (diff.getY() > 0) infos.side = EnumSide.BOTTOM;
							else infos.side = EnumSide.TOP;
						}
						if (absz > absy && absz > absx) {
							if (diff.getZ() > 0) infos.side = EnumSide.NORTH;
							else infos.side = EnumSide.SOUTH;
						}
						diff.dispose();
					}
					blockBB.dispose();
		...
		if (blockPos != null) blockPos.dispose();
		origin.dispose();
		pos.dispose();
		ray.dispose();
		rayBB.dispose();
	...
	public int getDirectElectricPowerAt(int x, int y, int z) {
		...
		for (EnumSide side : EnumSide.values()) {
			if (side == EnumSide.UNDEFINED) continue;
			...
			if (value != null && value instanceof EnumPowerStates) {
				...
				if (power.powerValue() == 15) return 15;
				else if (power.powerValue() > maxPower) maxPower = power.powerValue();
			}
		}
		return maxPower;
	}
	...
	public List<Entity> getEntitiesInRadius(float x, float y, float z, float f) {
		...
		for (Entity e : entities) {
			...
			if (entPos.sub(center).length() < f) result.add(e);
			entPos.dispose();
		}
		center.dispose();
		return result;
